...
public class ConsumeData extends BaseCommand {
    ...
    @CommandLine.Parameters(description = "The data to load", arity = "1..1")
    private String data;
    ...
    public void run() {
        ...
        if (dynamic) {
            learnService.consumeTextDynamic(source, id, data);
        } else {
            learnService.consumeTextStatic(data);
        }
    }
}
